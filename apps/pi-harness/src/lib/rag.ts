...
//
// Plumbing for the `rag_search` tool exposed in the harness's tool set
// (assembler.ts::toolsForProvider). Only registered when both
...
// can't be overridden per-test or per-process. Same trap that bit
// `WORKSPACE_ROOT` in v0.1.0 and `RAGFLOW_BASE_URL` in v0.3.0.
