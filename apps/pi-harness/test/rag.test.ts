/**
 * Tests for the rag-api client. Covers the four modes the AI SDK tool
 * wrapper has to handle: disabled config, network failure, HTTP error,
 * and a happy path with chunks. The client returns `{ error: string }`
 * rather than throwing so the tool wrapper can hand the message back to
 * the model instead of bailing the whole turn.
 *
 * Run with: bun test apps/pi-harness/test/rag.test.ts
 */
import { afterEach, beforeEach, describe, expect, it } from 'bun:test'
import { _resetWarnLatchForTests, getRagConfig, isRagEnabled, searchRag } from '../src/lib/rag'
