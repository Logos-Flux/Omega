...
//
// When `CF_ACCESS_TEAM_DOMAIN` and `CF_ACCESS_AUD` are set, the controller
// verifies the JWT that Cloudflare Access injects on every authenticated
// request and uses the claims (`email`, `sub`) as the user identity instead
// of falling back to the single-user stub in `session.ts`.
//
